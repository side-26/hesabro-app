<template lang="">
    <div @click="handleCheck(tarefehInfo.price)" :class="{'border-gray-200':!checked,'border-cyan-500':checked,'h-auto':!toggled,'h-fit':toggled}" class="bg-gray-100 cursor-pointer  p-3 md:p-5 border-2 flex-grow  mx-4 md:mx-0 rounded-lg transition-all   my-2">
        <section :class="{'text-cyan-500':checked,'mb-7':!toggled}" class="flex 0 transition-all flex-wrap  justify-between">
            <div class="flex justify-between flex-wrap items-center"><input  :class="{'border-gray-200':!checked,'border-cyan-500':checked}" :checked="checked"  class="ml-3 text-lg border-2 rounded-lg w-4 h-4" type="checkbox"   id=""><h4 class="font-IranYecan-extraBold text-sm">{{tarefehInfo.module_name}}</h4></div>
            <div class="items-center flex sm:mt-0 mt-5 text-center sm:text-left"><span class="font-IranYecan-extraBold">{{toPersian(seprateFu(tarefehInfo.price))}} تومان</span>
        ...
        <div class="py overflow-hidden" :class="{'h-0':toggled}">
            <ul>
                <li class="text-xs my-4 font-IranYecan-medium" v-for="(item,index) in itemArr" :key="item">{{toPersian(index+1)}}{{item}}
                </li>
            </ul>
        </div>
    </div>
