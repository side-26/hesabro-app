<template lang="">
  <div
    :class="{ 'sm:h-auto ': isClose, 'sm:h-fit': !isClose, 'mx-3 py-[0.922rem]': isSelected }"
    class="sm:cursor-pointer relative hover:z-[100] bg-gray-self md:bg-gray-200 px-4 py-[1.282rem] md:px-5 md:py-4 flex-grow sm:hover:-translate-y-1 sm:hover:shadow-md sm:hover:bg-white md:mx-0 my-3 rounded-lg transition-all"
  >
    <section class="flex transition-all items-center justify-between">
      ...
      <div class="items-center flex sm:mt-0 mt-3 text-center sm:text-left" @click.stop="handleSelect(tarefehInfo.price)">
        <button v-if="!isSelected" type="button" class="text-cyan-500 flex sm:flex-none justify-center md:hover:bg-cyan-500 ml-1 md:hover:text-white text-sm font-semibold sm:py-2 rounded-lg sm:justify-between items-center sm:px-3 transition-all">
