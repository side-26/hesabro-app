    ...
    <div @click="handleCheck(tarefehInfo.price)" :class="{'border-cyan-500 bg-white border-2':checked,'h-auto ':!toggled,'h-fit':toggled}" class=" cursor-pointer bg-gray-100  p-3 md:p-5  flex-grow  mx-4 md:mx-0 rounded-lg transition-all">
        <section :class="{'text-cyan-500 ':checked,'mb-3':!toggled}" class="flex 0 transition-all flex-wrap  justify-between">
            <div class="flex justify-between flex-wrap items-center">
                <div  :class="{'border-gray-300':!checked,'border-cyan-500 bg-cyan-500 border-0  after:w-4 after:h-1.5 after:text-white after:absolute after:border-b-3 after:border-l-3':checked}"   class="ml-3 relative border-2 w-5.5 h-5.5 rounded-md  after:-rotate-47 after:bottom-2  after:left-1">
                </div>
            <h4 class="font-IranYecan-bold text-sm md:text-base">{{tarefehInfo.module_name}}</h4>
            </div>
            <div class="items-center flex sm:mt-0  text-center sm:text-left"><span class="font-IranYecan-bold">{{toPersianNu(seprateNu(tarefehInfo.price))}} تومان</span>
        ...
        <div class="py overflow-hidden" :class="{'h-0':toggled}">
            <ul>
                <li class="text-xs my-5 font-IranYecan-medium" v-for="(item,index) in itemArr" :key="item">{{toPersianNu(index+1)}}{{item}}
                </li>
            </ul>
        </div>
    </div>
